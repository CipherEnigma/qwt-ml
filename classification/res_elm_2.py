...
def plot_quantization_error(x_input, y_full, y_quant, block_id, save_dir):
    """
    Plots visualization of the quantization error including Input(X) analysis.
    """
    # Convert to numpy and flatten
    x_flat = x_input.detach().cpu().float().numpy().flatten()
    y_flat = y_full.detach().cpu().float().numpy().flatten()
    yz_flat = y_quant.detach().cpu().float().numpy().flatten()
    
    error = y_flat - yz_flat
    
    # Sampling for speed/readability
    MAX_POINTS = 10000
    if len(y_flat) > MAX_POINTS:
        indices = np.random.choice(len(y_flat), MAX_POINTS, replace=False)
        y_plot = y_flat[indices]
        yz_plot = yz_flat[indices]
        error_plot = error[indices]
    else:
        y_plot = y_flat
        yz_plot = yz_flat
        error_plot = error
    ...
    # 1. Input vs Output (Linearity Check)
    axs[0].scatter(y_plot, yz_plot, alpha=0.1, s=1, color='blue')
    axs[0].plot([y_plot.min(), y_plot.max()], [y_plot.min(), y_plot.max()], 'r--', label='Ideal')
    axs[0].set_title(f'Block {block_id}: Output vs Quantized Output')
    axs[0].set_xlabel('Full Precision Output (y)')
    axs[0].set_ylabel('Quantized Output (yz)')
    axs[0].grid(True, alpha=0.3)

    # 2. Error Distribution (Gaussianity Check)
    axs[1].hist(error_plot, bins=100, color='purple', alpha=0.7)
    axs[1].set_title(f'Block {block_id}: Error Distribution')
    axs[1].set_xlabel('Error (y - yz)')
    axs[1].grid(True, alpha=0.3)

    # 3. Error vs Output Magnitude (Heteroscedasticity Check)
    axs[2].scatter(y_plot, error_plot, alpha=0.1, s=1, color='orange')
    axs[2].set_title(f'Block {block_id}: Error vs Output (y)')
    axs[2].set_xlabel('Output Value (y)')
    axs[2].set_ylabel('Error')
    axs[2].grid(True, alpha=0.3)

    # 4. Error vs Input Magnitude (The New Plot)
    # Note: Only possible if Input and Output sizes match (Stride=1, No Expansion)
    if x_flat.shape == error.shape:
        if len(x_flat) > MAX_POINTS:
            # Re-sample X using the SAME indices as error if shapes match exactly
            x_plot = x_flat[indices]
        else:
            x_plot = x_flat
            
        axs[3].scatter(x_plot, error_plot, alpha=0.1, s=1, color='green')
        axs[3].set_title(f'Block {block_id}: Error vs Input (X)')
        axs[3].set_xlabel('Input Value (x)')
        axs[3].set_ylabel('Error (y - yz)')
    else:
        # Placeholder if dimensions mismatch (e.g. Stride=2)
        axs[3].text(0.5, 0.5, f'Dim Mismatch\nIn: {x_flat.shape}\nOut: {error.shape}', 
    ...
    plt.tight_layout()
    save_path = os.path.join("/content/QwT/QwT-cls-RepQ-ViT/plots", f'block_{block_id}_analysis.png')
...
def generate_compensation_model_elm(q_model, train_loader, args):
    ...
    for sup_id in range(len(sup_layers)):
        ...
        for layer_id in range(len(current_sup_layer)):
            ...
            # --- PLOTTING (Updated with X) ---
            if args.local_rank == 0:
                plot_dir = os.path.join(args.log_dir, 'plots')
                os.makedirs(plot_dir, exist_ok=True)
                # Pass output_t_ as the First Argument (X)
                plot_quantization_error(output_t_, output_full_precision, output_quant, global_layer_id, plot_dir)
            # ---------------------------------
